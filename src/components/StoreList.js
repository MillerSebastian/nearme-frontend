export class StoreList {
      ...
      if (stores.length === 0) {
        return `
          <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-slate-300">No se encontraron tiendas</h3>
            <p class="mt-1 text-sm text-slate-400">
              ${
                searchQuery
                  ? `No hay tiendas que vendan "${searchQuery}" en tu área.`
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                  <span class="text-lg font-bold text-green-400">Tienda registrada</span>
  ...
  // Funciones globales para manejar acciones de tienda
  window.viewStoreDetails = async (storeNit) => {
    try {
      ...
      // Combinar la información de la tienda con los productos
      const combinedData = {
        ...storeData,
        products: productsData.products || productsData, // Manejar ambas estructuras
    ...
    modal.innerHTML = `
      <div class="card max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-white">Detalles de la Tienda</h3>
        ...
        <div class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h4 class="text-lg font-semibold text-white mb-2">Información de la Tienda</h4>
              <div class="space-y-2 text-sm">
                <p><span class="text-slate-400">Nombre:</span> <span class="text-white">${
                  data.store_name || "No disponible"
                }</span></p>
                ...
                <p><span class="text-slate-400">Teléfono:</span> <span class="text-white">${
                  data.phone_number || "No disponible"
                }</span></p>
                ...
                <p><span class="text-slate-400">Visualizaciones:</span> <span class="text-white">${
                  data.total_views || 0
                }</span></p>
                <p><span class="text-slate-400">Productos:</span> <span class="text-white">${
          ...
          ${
            ...
            data.products.length > 0
              ? `
            <div>
              <h4 class="text-lg font-semibold text-white mb-4">Productos Disponibles</h4>
          ...
          `
            ...
            <div class="text-center py-8">
              <p class="text-slate-400">Esta tienda aún no tiene productos registrados.</p>
  ...
  function showContactModal(store) {
    ...
    modal.innerHTML = `
      <div class="card max-w-md w-full">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-white">Contactar ${
            store.store_name
          }</h3>
          <button onclick="this.closest('.fixed').remove()" class="text-slate-400 hover:text-white">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-4">
          <div>
            ...
            <p class="text-slate-300">${store.phone_number || "No disponible"}</p>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-white mb-2">Email</label>
            <p class="text-slate-300">${store.email || "No disponible"}</p>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-white mb-2">Dirección</label>
            <p class="text-slate-300">${store.address || "No disponible"}</p>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-white mb-2">Horario</label>
